...
const Products = () => {
    ...
    useEffect(() => {
        ...
        if (pathSegments[0] === 'categorie-produs' && pathSegments.length >= 3) {
            ...
            const category = categories && categories.find(cat => removeDiacritics(cat.categoryType).toLowerCase() === decodedCategoryPath.replace(/-/g, ' '));
            if (category) {
                categoryActive = category.categoryType;

                const subcategory = category.categorySet.find(sub => removeDiacritics(sub.categoryName).toLowerCase() === decodedSubcategoryPath.replace(/-/g, ' '));
                if (subcategory) {
        ...
        const initialFilters: Partial<FiltersState> = {
            ...
            companiesSelected: queryParameters.get('companies') ? queryParameters.get('companies')?.split(',') : [],
        };
            ...
            <div className={cls.productsWrapper}>
                {isCategoryLoading ? (
                    ...
                    <>
                            ...
                            {categories &&
                                categories.map((item, index) => (
                                    <CategoryView
                                        ...
                                        handleCategoryChange={(category, subcategory, subCategoryId) => {
                                            handleFiltersChange({ categoryActive: category, subcategoryActive: subcategory, categoryId: subCategoryId });
                        ...
                        {filters.categoryId ? <span>Produse &rarr; {filters.categoryActive} &rarr; {filters.subcategoryActive}</span> : <></>}
                        <div className={cls.contentWrapper}>
                            {((isFilterOpen && isMobile) || !isMobile) && (
                                <Filter
                                    search={filters.searchValue}
                                    handleSearch={(value: string) => handleFiltersChange({ searchValue: value })}
                                    companiesSelected={filters.companiesSelected}
                                    handleCompaniesSelected={(companies: string[]) => handleFiltersChange({ companiesSelected: companies })}
                                    minPrice={filters.minPrice}
                                    handleMinPrice={(value: string) => handleFiltersChange({ minPrice: value })}
                                    maxPrice={filters.maxPrice}
                                    handleMaxPrice={(value: string) => handleFiltersChange({ maxPrice: value })}
                                />
                            )}
                            {products.length || productLoading ? (
                                <div className={cls.productsDataWrapper}>
                                    ...
                                    {!productsFinished ? (
                                        <div ref={lastProductElementRef} style={isMobile ? {
                                            display: 'flex',
